...
void main() {
    ...
    if (PASSINDEX == 0) {
      ...
      float r = pow(sin(length(m - uv) * 3.0 - time - (rr + 1.5)), 22.0) ;
      float g = pow(sin(length(m - uv) * 2.0 - time + gg), 20.);
      float b = pow(sin(length(m - uv) * 1.0 - time + bb), 3.0);

      vec3 bw = vec3(r + g + b) / 2.0;
      gl_FragColor = vec4(r,g,b, 0.02)+ statue;
    }
}
